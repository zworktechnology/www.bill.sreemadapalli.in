    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <form autocomplete="off" method="POST"
                action="{{ route('productsession.edit', ['id' => $Productdatas['id']]) }}">
                @csrf
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control  select product_id" name="product_id" id="product_id" disabled>
                                ...
                                @foreach ($Product as $Products)
                                    <option
                                        value="{{ $Products->id }}"@if ($Products->id === $Productdatas['id']) selected='selected' @endif>
                                    ...
                                    <div class="input-group produtseesiondiv" style="margin-right: 5px;height: 37px;">
                                        <div class="input-group-text" style="background: #dc3545;padding: 0px;">
                                            <button style="width: 10px;"
                                                class="text-white py-1 font-medium rounded-lg text-sm  text-center btn btn-danger remove-produtseesiondiv"
                                                type="button">-</button>
                                            <input type="hidden" name="productsession_id[]" id="productsession_id"
                                                value="{{ $terms_array['id'] }}" />
                                        </div>
                                        <select class="form-control  select session_id" name="session_id[]"
                                            id="session_id">
                                            <option value="" disabled selected hiddden>Select Session</option>
                                            @foreach ($session as $sessions)
                                                <option
                                                    value="{{ $sessions->id }}"@if ($sessions->id === $terms_array['session_id']) selected='selected' @endif>
